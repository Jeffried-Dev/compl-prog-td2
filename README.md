...
1.  Quelles conventions utilise Maven pour l’arborescence des répertoires ?
    > les fichiers sont dans le repertoire 'src' qui lui contient une partie "main" c'est dans cette partie qu'on range les fichiers du projet et une partie test ou on range les fichiers de tests.
    nous avons aussi un repertoire "target" generer par MVN.
1.  Examinez le projet Maven ainsi que le fichier de description et répondez aux questions suivantes :
    1.  Dans quel répertoire se trouvent les fichiers sources du projet ? Les sources des tests ?
        > les fichiers sources se trouvent dans src/main, les fichiers test se trouvent dans src/test
    ...
    1. À quoi sert la propriété `project.build.sourceEncoding` ?
        >  spécifier explicitement l'encodage de caractères que le projet utilise pour les fichiers source.
    ...
    1. Quelle version de JUnit est configurée ? À quoi sert la balise `scope` ?
        > 4.11, "scope" permet de spécifier la portée d'une dépendance dans le projet.
    1. À quoi sert la section `pluginManagement` ?
        >  permet de définir des configurations de plugins de manière centralisée pour être partagées entre plusieurs projets sans avoir besoin de les répéter dans chaque projet.
